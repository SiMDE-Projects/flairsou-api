...
class ReconciliationSerializer(FlairsouModelSerializer):
    ...
    def check_date_after_last_reconc(
        self, account: Account, date: datetime.date
    ) -> None:
        """
    ...
    def check_balance_valid(
        self, account: Account, date: datetime.date, balance: int
    ) -> None:
        """
        ...
        if balance != account.balance_at_date(date):
            raise self.ValidationError(
                {"balance": ReconciliationSerializer.error_messages["balance"]}
            )
    ...
    def check_account_nonvirtual(self, account: Account) -> None:
        ...
        if account.virtual:
            raise self.ValidationError(
                {"account": ReconciliationSerializer.error_messages["account"]}
            )
