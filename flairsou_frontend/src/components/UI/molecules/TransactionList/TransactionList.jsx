...
const TransactionList = ({ accountID, accountType, updateBalanceCallback }) => {
  ...
  useEffect(() => {
    ...
    fetch(process.env.BASE_URL.concat(`api/accounts/${accountID}/transactions/`))
  ...
  const deleteTransaction = (transactionPk) => {
    fetch(process.env.BASE_URL.concat(`api/transactions/${transactionPk}/`), { method: 'DELETE' })
      .then((response) => {
  ...
  const updateTransaction = (transaction, resetTransactionModifiedStatus) => {
    fetch(process.env.BASE_URL.concat(`api/transactions/${transaction.pk}/`),
      {
        method: 'PUT',
