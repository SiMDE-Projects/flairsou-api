...
class Asso(models.Model):
    ...
    shortname = models.CharField("shortname", max_length=100, blank=False, null=False)

    # nom complet de l'association
    name = models.CharField("name", max_length=100, blank=False, null=False)

    # type de l'association
    asso_type = models.IntegerField(choices=AssoType.choices, blank=False, null=False)

    # association parente (pôle, BDE ou rien)
    parent = models.ForeignKey(
        "self", on_delete=models.DO_NOTHING, blank=False, null=True
    )

    # autorisation donnée à l'association parente pour consulter la
    # comptabilité de l'asso courante
    parent_view_granted = models.BooleanField(
        "Parent view granted", blank=False, null=False, default=False
    )

    # indique si l'association est dans le cimetière ou non (potentiellement
    # pour passer en lecture seule les comptes)
    in_cemetery = models.BooleanField(
        "in_cemetery", blank=False, null=False, default=False
    )

    # date de dernière mise à jour de l'association, pour savoir si il faut
    # la mettre à jour
    last_updated = models.DateTimeField(name="last_updated", blank=False, null=False)
    ...
    def get_sub_assos_granted(self):
        ...
        """
        return self.asso_set.filter(
            ~models.Q(asso_type=AssoType.ASS1901) | models.Q(parent_view_granted=True)
        )
    ...
    def create_asso(cls, PDA_resp):
        ...
        if PDA_resp["type"]["id"] not in asso_type_id_to_enum.keys():
            ...
            raise ValueError(
                "Le type d'association {} n'est pas connu".format(
                    PDA_resp["type"]["name"]
                )
            )
